...
/*
  We need to check for proxytypes before we draw shit
*/
...
int main() {
    ...
    // Add camera
    Camera = irrScene->addCameraSceneNodeFPS(0, 100, 10);
    Camera->setPosition(vector3df(0, 5, -5));
    Camera->setTarget(vector3df(0, 0, 0));

    // Preload textures
    irrDriver->getTexture("assets/grass.png");
    irrDriver->getTexture("assets/lava.png");
...
void CreateBox(const btVector3 &TPosition, const vector3df &TScale, btScalar TMass) {
    ...
    Node->setMaterialTexture(0, irrDriver->getTexture("assets/lava.png"));
...
void CreateSphere(const btVector3 &TPosition, btScalar TRadius, btScalar TMass) {
    ...
    Node->setMaterialFlag(EMF_LIGHTING, 1);
    Node->setMaterialFlag(EMF_NORMALIZE_NORMALS, true);
    Node->setMaterialTexture(0, irrDriver->getTexture("assets/grass.png"));
    ...
    float xv = sin(Camera->getRotation().Y * M_PI / 180.0f) * cos(Camera->getRotation().X * M_PI / 180.0f) * 10.0f;
    float yv = -1 * sin(Camera->getRotation().X * M_PI / 180.0f) * 10.0f;
    float zv = cos(Camera->getRotation().Y * M_PI / 180.0f) * cos(Camera->getRotation().X * M_PI / 180.0f) * 10.0f;
